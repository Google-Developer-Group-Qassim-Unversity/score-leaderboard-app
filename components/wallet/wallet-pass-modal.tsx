...
export function WalletPassModal({ isOpen, onClose, data }: WalletPassModalProps) {
  ...
  const handleAppleWallet = async () => {
    ...
    try {
      ...
      toast.error("حدث خطأ أثناء تحميل بطاقة آبل.")
    } finally {
  ...
  const handleGoogleWallet = async () => {
    ...
    try {
      ...
      toast.error("حدث خطأ أثناء إنشاء بطاقة Google Wallet")
    } finally {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="sm:max-w-md p-0 overflow-hidden bg-card border-border/80 rounded-3xl" dir="rtl">
        <div className="p-6 sm:p-8 space-y-6">
          {/* Header Banner */}
          <DialogHeader className="text-center space-y-2">
            <div className="mx-auto w-12 h-12 rounded-2xl bg-emerald-500/10 text-emerald-500 flex items-center justify-center mb-1">
            ...
            <DialogDescription className="text-xs text-muted-foreground">
              أضف بطاقتك الآن إلى محفظة جوالك وشارك رابط صفحتك الشخصية المعتمدة
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-3 pt-2">
            ...
            <button
              ...
              className="w-full h-14 bg-black text-white hover:bg-neutral-900 border border-neutral-800 rounded-2xl flex items-center justify-center gap-3 px-6 shadow-md transition-all active:scale-[0.98] group disabled:opacity-75 cursor-pointer"
            >
              {isGeneratingApplePass ? (
                ...
                <>
                  ...
                  <Download className="w-4 h-4 mr-auto opacity-70 group-hover:opacity-100 transition-opacity" />
                </>
              )}
            </button>
            ...
            <button
              ...
              className="w-full h-14 bg-white text-slate-900 hover:bg-slate-50 border border-slate-200 dark:border-slate-700 rounded-2xl flex items-center justify-center gap-3 px-6 shadow-md transition-all active:scale-[0.98] group disabled:opacity-75 cursor-pointer"
            >
              {isGeneratingGooglePass ? (
                ...
                <>
                  ...
                  <ExternalLink className="w-4 h-4 mr-auto opacity-70 group-hover:opacity-100 text-slate-600 transition-opacity" />
                </>
              )}
            </button>
          </div>
          ...
          <div className="pt-2 border-t border-border/60 space-y-2.5">
            ...
            <div className="flex items-center gap-2 p-2.5 rounded-2xl bg-muted/50 border border-border/80">
              <div className="text-xs font-mono font-medium truncate flex-1 text-left select-all text-muted-foreground" dir="ltr">
                {fullProfileUrl}
              </div>
              <Button
                size="sm"
          ...
          <div className="pt-1">
            <Link href={profilePath} onClick={onClose} className="block w-full">
              <Button variant="ghost" className="w-full text-xs font-bold gap-2 text-primary hover:text-primary">
                <span>فتح ومعاينة ملفك الشخصي العام</span>
