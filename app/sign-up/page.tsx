...
function SignUpContent() {
  ...
  const handleVerificationSuccess = async (sessionId: string) => {
    ...
    await setActive({
      session: sessionId,
      navigate: async () => {
        router.push(withRedirectParam('/onboarding', redirectParam))
      },
    })
  }
  ...
  const handleGoogleSignUp = async () => {
    ...
    try {
      await signUp.authenticateWithRedirect({
        ...
        redirectUrl: withRedirectParam('/sign-up/sso-callback', redirectParam),
      ...
      <Card className="w-full max-w-md border-t-4 border-t-green-600">
        ...
        <CardFooter className="flex flex-col space-y-2">
          <div className="text-sm text-center text-muted-foreground">
            ...
            <Link
              href={withRedirectParam('/sign-in', redirectParam)}
