export type Language = 'en' | 'ar';

export const resources = {
  en: {
    translation: {
      // Hero Section
      "hero.title.performance": "GDG on Campus",
      "hero.subtitle": "Join our events, track your points, and climb to the top of the leaderboard! 🏆",
      "hero.viewAllMembers": "View All Members",
      "hero.viewDepartments": "View Departments",

      // Stats Cards
      "stats.members.title": "Members",
      "stats.members.description": "Competing participants",
      "stats.members.badge": "Active",
      "stats.departments.title": "Departments",
      "stats.departments.description": "Our Strong Teams",
      "stats.departments.badge": "Teams",
      "stats.points.title": "Points",
      "stats.points.description": "Points earned",
      "stats.points.badge": "Total",
      "stats.events.title": "Events",
      "stats.events.description": "Total events",
      "stats.events.badge": "Active",

      // Leaderboard Section
      "leaderboard.badge": "Live Rankings",
      "leaderboard.title": "Current Leaders",
      "leaderboard.subtitle": "Check out the top competitors across members and departments. Updated in real-time!",
      "leaderboard.topMembers": "Top Members",
      "leaderboard.topDepartments": "Top Departments",
      "leaderboard.viewAll": "View All",
      "leaderboard.specializedDepts": "Specialized departments",
      "leaderboard.administrativeDepts": "Administrative departments",
      "leaderboard.points": "Points",
      "leaderboard.viewDetails": "View Details",
      "leaderboard.details": "Details",
      "leaderboard.noMembersData": "No members data available",
      "leaderboard.noSpecializedDeptsData": "No specialized departments data available",
      "leaderboard.noAdministrativeDeptsData": "No administrative departments data available",
      "leaderboard.showDetails": "Show Details",
      "leaderboard.hideDetails": "Hide Details",
      "leaderboard.show": "Show",
      "leaderboard.hide": "Hide",

      // Semester
      "semester.label": "Semester",
      "semester.all": "All",

      // Footer CTA
      "footer.title": "Ready to Climb the Ranks?",
      "footer.subtitle": "Join events, earn points, and see your name rise on the leaderboard",
      "footer.button": "View Upcoming Events",

      // Events Section
      "events.badge": "Open Events",
      "events.title": "Join Our Events",
      "events.subtitle": "Experience the excitement: Join, earn points, and lead the board!",
      "events.loading": "Loading events...",
      "events.empty": "No events are currently available, but stay tuned — we have plenty on the way! 😉",
      "events.viewAll": "View All Events",

      // Navigation
      "nav.home": "Home",
      "nav.members": "Members",
      "nav.departments": "Departments",
      "nav.events": "Events",
      "nav.wallet": "Wallet",
      "nav.stats": "Stats",
      "nav.magazines": "Magazines",
      "nav.structure": "Structure",
      "nav.howItWorks": "Points System",
      "nav.language": "Language",
      "nav.english": "English",
      "nav.arabic": "Arabic",
      "nav.profile": "Profile",
      "nav.admin": "Admin Panel",

      // Profile Page
      "profile.pageTitle": "Profile Center",
      "profile.gdgProfile": "GDG Profile",
      "profile.accountManagement": "Account Management",
      "profile.eventsFeed": "Events Feed",
      "profile.eventsFeedPlaceholder": "Events feed coming soon...",
      "profile.attendedTitle": "Attended Events",
      "profile.participatedTitle": "Competitions & Events You Joined",
      "profile.noAttended": "No attended events yet",
      "profile.noParticipated": "No participated events yet",
      "profile.downloadCertificate": "Certificate",
      "profile.customize": "Customize",
      "profile.language": "Language",
      "profile.format": "Format",
      "profile.download": "Download",
      "profile.langArabic": "Arabic",
      "profile.langEnglish": "English",
      "profile.formatPdf": "PDF",
      "profile.formatPng": "PNG",
      "profile.downloading": "Downloading...",
      "profile.downloadSuccess": "Certificate downloaded successfully!",
      "profile.downloadError": "Failed to download certificate. Please try again.",
      "profile.certificateDisabledTooltip": "You must attend all days to download the certificate",
      "profile.details": "Details",
      "profile.attendance": "Attendance",
      "profile.day": "Day",
      "profile.attended": "Attended",
      "profile.notAttended": "Not attended",

      // Members Page
      "members.backButton": "Back to Dashboard",
      "members.heading": "Members Leaderboard",
      "members.subHeading": "members ranked by total points earned through various activities and achievements",
      "members.findMembers": "Find Members",
      "members.searchPlaceholder": "Search members by name...",
      "members.rankings": "Rankings",
      "members.performance": "Individual member performance rankings",
      "members.showingTop": "Showing top 100 members",
      "members.of": "of",
      "members.found": "members found",
      "members.noResults": "No members found matching",
      "members.clearSearch": "Clear Search",
      "members.noMembers": "No members available",
      "members.viewYourRank": "View Your Rank",

      // Events Page
      "events.heading": "Events",
      "events.subHeading": "Discover upcoming events, sign up for open registrations, and explore past events",
      "events.openEvents": "Open Events",
      "events.pastEvents": "Past Events",
      "events.noOpenEvents": "There are currently no events available for signups. Check back soon!",
      "events.noPastEvents": "Past events will appear here once they are completed.",
      "events.SearchEvents": "Search events...",
      "events.noResults": "No events match your search.",

      // Departments Page
      "departments.backButton": "Back to Dashboard",
      "departments.heading": "Departments Leaderboard",
      "departments.subHeading": "departments ranked by total points earned through team collaboration and achievements",
      "departments.administrative": "Administrative Departments",
      "departments.administrativeDesc": "Support and management teams",
      "departments.specialized": "Specialized Departments",
      "departments.specializedDesc": "Hands-on and technical teams",

      // Magazines Page
      "magazines.backButton": "Back to Dashboard",
      "magazines.heading": "GDG Magazines",
      "magazines.noMagazines.title": "No magazines available",
      "magazines.noMagazines.desc": "Check back soon for new magazine releases featuring the latest in technology and development.",
      "magazines.pages": "pages",
      "magazines.readPDF": "Read PDF",
      "magazines.reader.backToMagazines": "Back to Magazines",
      "magazines.reader.title": "Magazine Reader",
      "magazines.reader.loading": "Loading reader...",
      "magazines.reader.notFound": "Magazine not found",

      // Magazines Section (Home)
      "magazines.section.title": "Latest Magazines",
      "magazines.section.subtitle": "Explore our latest publications and stay updated with the community",
      "magazines.section.viewAll": "View All",
      "magazines.section.empty": "No magazines available yet",

      // Club Structure Section
      "clubStructure.title": "Our Club Structure",
      "clubStructure.subtitle": "Meet our leadership team and explore the departments driving our community",
      "clubStructure.leadership": "Club Leadership",
      "clubStructure.presidents": "Club Leaders",
      "clubStructure.vicePresident": "Vice President",
      "clubStructure.departmentsTitle": "Departments",
      "clubStructure.departments": "Departments",
      "clubStructure.specialized": "Specialized",
      "clubStructure.administrative": "Administrative",
      "clubStructure.viewFullStructure": "View Full Structure",

      // Club Structure Page
      "clubStructurePage.title": "Club Structure",
      "clubStructurePage.subtitle": "Learn about the organizational structure of the Google Developers Club and its different departments",
      "clubStructurePage.leadership": "Leadership",
      "clubStructurePage.specializedDepts": "Specialized Departments",
      "clubStructurePage.specializedDesc": "Departments specialized in technology and innovation fields",
      "clubStructurePage.administrativeDepts": "Administrative Departments",
      "clubStructurePage.administrativeDesc": "Administrative departments to organize and develop club activities",
      "clubStructurePage.deptHead": "Department Head:",
      "clubStructurePage.deptHeadMale": "Department Head (Male)",
      "clubStructurePage.deptHeadFemale": "Department Head (Female)",
      "clubStructurePage.deptMembers": "Department Members:",
      "clubStructurePage.presidents": "Club Leaders",
      "clubStructurePage.vicePresident": "Vice President",

      // OLD specialized Departments
      "clubStructurePage.robotics": "Robotics",
      "clubStructurePage.ai": "Artificial Intelligence and Data Analysis",
      "clubStructurePage.entrepreneurship": "Entrepreneurship and Technology",
      "clubStructurePage.esports": "Esports",

      // NEW specialized Departments
      "clubStructurePage.blue": "Blue",
      "clubStructurePage.green": "Green",
      "clubStructurePage.yellow": "Yellow",

      // Administrative Departments
      "clubStructurePage.development": "Technical Development",
      "clubStructurePage.design": "Design",
      "clubStructurePage.programs": "Programs and Logistics Management",
      "clubStructurePage.organization": "Organization and Meetings",
      "clubStructurePage.media": "Media and Public Relations",
      "clubStructurePage.operations": "Operations Management",
      "clubStructurePage.activities": "General Activities",
      
      "clubStructurePage.deptLeader": "Leader:",
      "clubStructurePage.deptVice": "Vice:",
      "clubStructurePage.aiShort": "AI & Data",
      "clubStructurePage.programsShort": "Programs & Logistics",
      "clubStructurePage.boardDirectors": "Board of Directors",
      "clubStructurePage.innovation": "Innovation Department",
      "clubStructurePage.loadError": "We couldn't load the club structure. Please try again.",
      "clubStructurePage.tryAgain": "Try again",
      "clubStructurePage.noAssignments": "No one has been assigned yet",
      "clubStructurePage.noMembers": "No members have been assigned yet",
      "clubStructurePage.noDepartments": "No departments are available yet",

      // Auth
      "auth.login": "Log In",
      "auth.signup": "Sign Up",
      "auth.loginAria": "Sign in to your account",
      "auth.signupAria": "Create a new account",
      "auth.signIn": "Sign In",
      "auth.joinUs": "Join Us",
      "auth.cancel": "Cancel",

      // Auth Required Dialog
      "authDialog.defaultTitle": "Sign In Required",
      "authDialog.defaultDescription": "You need to sign in or create an account to continue.",

      // Event Signup
      "eventSignup.signUp": "Sign Up",
      "eventSignup.signedUp": "Signed Up",
      "eventSignup.fillGoogleForm": "Fill Google Form",
      "eventSignup.somethingWentWrong": "⚠️ Something went wrong",
      "eventSignup.signInRequired": "Sign In Required",
      "eventSignup.signInDescription": "You need to sign in or create an account to sign up for this event.",
      "eventSignup.confirmTitle": "Confirm Sign Up",
      "eventSignup.confirmDescription": "Are you sure you want to sign up for this event:",
      "eventSignup.cancel": "Cancel",
      "eventSignup.signingUp": "Signing up...",
      "eventSignup.confirmAndFillForm": "Confirm Sign Up and Fill Form",
      "eventSignup.confirm": "Confirm Sign Up",
      "eventSignup.successToast": "Successfully signed up for",

      // Event Card
      "eventCard.viewEvent": "View Event",
      "eventCard.viewDetails": "View Details",

      // Attendance Page
      "attendance.processing": "Processing...",
      "attendance.markingAttendance": "Marking attendance...",
      "attendance.confirmed": "Attendance Confirmed",
      "attendance.failed": "Attendance Failed",
      "attendance.invalidLink": "Invalid Link",
      "attendance.signInRequired": "Sign In Required",
      "attendance.success": "Your attendance has been marked successfully!",
      "attendance.noToken": "No attendance token provided. Please use the QR code or link provided by the event organizer.",
      "attendance.pleaseSignIn": "Please sign in to mark your attendance.",
      "attendance.notEligible": "You are not eligible to mark attendance for this event.",
      "attendance.expired": "The attendance link has expired or is invalid. Please request a new one from the event organizer.",
      "attendance.serverError": "A server error occurred. Please try again later.",
      "attendance.connectionError": "Failed to connect to the server. Please check your internet connection and try again.",
      "attendance.dialog.title": "Sign In Required",
      "attendance.dialog.description": "You need to sign in or create an account to mark your attendance.",

      // Event Detail Page
      "eventDetail.status.open": "Open",
      "eventDetail.status.openToJoin": "Open to Join",
      "eventDetail.status.active": "Active",
      "eventDetail.status.closed": "Closed",
      "eventDetail.locationType.online": "Online Event",
      "eventDetail.locationType.onsite": "On-site Event",
      "eventDetail.locationType.none": "No Location",
      "eventDetail.daily": "daily",
      "eventDetail.days": "Days",
      "eventDetail.description": "Description",
      "eventDetail.noDescription": "No description provided for this event.",
      "eventDetail.noImage": "No event image",
      "eventDetail.joinMeeting": "Join Meeting",
      "eventDetail.meetingLink": "Meeting Link",
      "eventDetail.addToCalendar": "Add to Google Calendar",

      // Member Detail Page
      "memberDetail.backToMembers": "Back to Members",
      "memberDetail.memberProfile": "Member Profile",
      "memberDetail.member": "Member",
      "memberDetail.totalPoints": "Total Points",
      "memberDetail.pointsHistory": "Points History",
      "memberDetail.detailedLog": "Detailed log of all points earned",
      "memberDetail.points": "points",
      "memberDetail.noHistory": "No points history available for this member.",

      // Department Detail Page
      "departmentDetail.backToDepartments": "Back to Departments",
      "departmentDetail.departmentProfile": "Department Profile",
      "departmentDetail.department": "Department",
      "departmentDetail.totalPoints": "Total Points",
      "departmentDetail.pointsHistory": "Points History",
      "departmentDetail.detailedLog": "Detailed log of all points earned by the department",
      "departmentDetail.points": "points",
      "departmentDetail.noHistory": "No points history available for this department.",

      // Footer
      "footer.about.title": "About GDG Qassim",
      "footer.about.description": "GDG Qassim is a student-led community fostering innovation and technical skills through workshops, events, and collaborative projects.",
      "footer.quickLinks.title": "Quick Links",
      "footer.connect.title": "Connect With Us",
      "footer.developers.title": "Developers",
      "footer.developers.builtWith": "Built with ❤️ by",
      "footer.social.joinUs": "Join Our Community",
      "footer.social.telegram": "Telegram Group",
      "footer.social.discord": "Discord Server",
      "footer.social.twitter": "X",
      "footer.social.hashtag": "Our X Hashtag",
      "footer.social.tiktok": "TikTok",
      "footer.social.email": "Contact Us",
      "footer.social.gdgSite": "Official GDG Site",
      "footer.copyright": "© GDG-Q 2026",
      "footer.privacy": "Privacy Policy",
      "footer.terms": "Terms of Service",

      // Feedback
      "feedback.card.title": "Your Feedback Matters",

      // Not Found Pages
      "notFound.pageTitle": "Page Not Found",
      "notFound.pageDescription": "The page you're looking for doesn't exist or has been moved.",
      "notFound.backHome": "Back to Home",
      
      "notFound.eventTitle": "Event Not Found",
      "notFound.eventDescription": "The event you're looking for doesn't exist or may have been removed.",
      "notFound.backToEvents": "Back to Events",

      // How / Points System Page
      "how.title": "Points Evaluation and Tracking System",
      "how.subtitle": "for GDG on Campus",
      "how.generalIdea": "General Idea",
      "how.generalIdea.desc": "GDG points reflect members' and departments' contributions, where every effort and achievement is recognized clearly. Your participation makes a difference, and every member has a unique role and a special impact.",
      "how.monthly": "Monthly",
      "how.monthly.topMembers": "Three Members of the Month",
      "how.monthly.topDept": "Creative Department of the Month",
      "how.endOfTerm": "End of Semester",
      "how.endOfTerm.top5": "Honoring top 5 members (expandable to 10)",
      "how.endOfTerm.specialized": "Honoring the Outstanding Specialized Department",
      "how.endOfTerm.administrative": "Honoring the Outstanding Administrative Department",
      "how.note.multipliers": "Each activity has base points + multipliers (based on attendance, quality, and timeliness, etc.).",
      "how.note.bothTypes": "There are department points and member points separately.",
      "how.memberPoints": "Member Points",
      "how.memberPoints.subtitle": "Attendance / Participation / Organization",
      "how.tableHeader.action": "Action",
      "how.tableHeader.points": "Points",
      "how.deptPoints": "Department Points",
      "how.deptPoints.subtitle": "Activity Types and Base Points",
      "how.tableHeader.type": "Type",
      "how.tableHeader.definition": "Definition",
      "how.memberAction.0": "Attending an on-site course",
      "how.memberAction.1": "Attending an online course",
      "how.memberAction.2": "Attending a bootcamp (per day)",
      "how.memberAction.3": "Attending an online bootcamp (per day)",
      "how.memberAction.4": "Attending a large event (300+ attendees)",
      "how.memberAction.5": "Attending a tech talk/session",
      "how.memberAction.6": "Participating in an external competition",
      "how.memberAction.7": "Qualifying/placing in a competition",
      "how.memberAction.8": "Winning first place externally",
      "how.memberAction.9": "Giving a short presentation (≤ 20 min)",
      "how.memberAction.10": "Giving a full course",
      "how.memberAction.11": "Organizing/volunteering at an event (per shift)",
      "how.memberAction.12": "Contributing an approved design",
      "how.memberAction.13": "Writing an approved article/tutorial video",
      "how.memberAction.14": "Participating in an announced practical project",
      "how.deptType.0": "On-site Course",
      "how.deptType.1": "Online Course",
      "how.deptType.2": "Training Bootcamp",
      "how.deptType.3": "Online Bootcamp",
      "how.deptType.4": "Large Event (300+ attendees)",
      "how.deptType.5": "Twitter Space",
      "how.deptType.6": "Tech Talk/Monthly Session",
      "how.deptType.7": "Internal Competition (Organizing)",
      "how.deptType.8": "External Competition Participation",
      "how.deptType.9": "Practical Project/Product",
      "how.deptType.10": "Promotional Event/Booth",
      "how.deptType.11": "Partnership/Sponsorship",
      "how.deptType.12": "Published Educational Content",
      "how.deptDesc.0": "Organizing an on-site course",
      "how.deptDesc.1": "Organizing a remote course",
      "how.deptDesc.2": "On-site bootcamp (per day)",
      "how.deptDesc.3": "Online training bootcamp (per day)",
      "how.deptDesc.4": "A large event with 300+ people",
      "how.deptDesc.5": "Hosting an audio space on Twitter",
      "how.deptDesc.6": "Knowledge exchange/discussion session",
      "how.deptDesc.7": "Competition organized by the department",
      "how.deptDesc.8": "Counted once per representing team",
      "how.deptDesc.9": "Delivering a project with public repo + presentation",
      "how.deptDesc.10": "Club introduction corner at the university",
      "how.deptDesc.11": "Agreement supporting activity/prizes/venue",
      "how.deptDesc.12": "Article/video under the club's identity",

      // Profile Form
      "profileForm.uniId": "University ID",
      "profileForm.name": "Name",
      "profileForm.name.placeholder": "Example: Ibrahim Mohammed Bassam Al-Harbi",
      "profileForm.phone": "Phone Number",
      "profileForm.gender": "Gender",
      "profileForm.gender.male": "Male",
      "profileForm.gender.female": "Female",
      "profileForm.level": "Academic Level",
      "profileForm.level.placeholder": "Select Level",
      "profileForm.level.graduated": "Graduated",
      "profileForm.college": "College",
      "profileForm.college.placeholder": "Select College",
      "profileForm.collegeOther": "College Name",
      "profileForm.collegeOther.placeholder": "Enter your college name",
      "profileForm.email": "Personal Email",
      "profileForm.email.hint": "Personal email, not the one ending with @qu.edu.sa",
      "profileForm.email.lockedHint": "This is your Google sign-in email and can't be changed here.",
      "profileForm.locked": "Locked",
      "profileForm.uniId.notLinked": "Not linked (signed in with Google)",
      "profileForm.saving": "Saving...",
      "profileForm.save": "Save Changes",
      "walletTheme.label": "Card Color",
      "walletTheme.success": "Card color updated",
      "walletTheme.error": "Couldn't update your card color, please try again",
      "profileForm.errors.nameRequired": "Name is required",
      "profileForm.errors.phoneRequired": "Phone number is required",
      "profileForm.errors.phoneInvalid": "Phone must start with 05 and be 10 digits",
      "profileForm.errors.collegeOtherRequired": "Please enter your college name",
      "profileForm.errors.emailRequired": "Email is required",
      "profileForm.errors.emailInvalid": "Invalid email address",
      "profileForm.errors.emailUniversity": "Personal email cannot be a @qu.edu.sa address",
      "profileForm.toast.clerkUpdatedBackendFailed": "Clerk updated but backend sync failed.",
      "profileForm.toast.emailConflict": "This email is already in use by another account. Please use a different email.",
      "profileForm.toast.success": "Profile updated successfully!",
      "profileForm.toast.error": "An unexpected error occurred. Please try again.",

      // Error Display
      "error.connection": "Connection Error",
      "error.notFound": "Not Found",
      "error.unauthorized": "Unauthorized",
      "error.error": "Error",
      "error.somethingWentWrong": "Something went wrong",
      "error.connectionMessage": "Please check your internet connection and try again.",
      "error.notFoundMessage": "The requested resource could not be found.",
      "error.unauthorizedMessage": "You need to be signed in to view this content.",
      "error.unexpectedMessage": "An unexpected error occurred.",
      "error.tryAgain": "Try again",

      // Google Form
      "googleForm.noFormUrl": "No form URL provided",
      "googleForm.loading": "Loading…",

      // Stats Page
      "stats.title": "Season in Review",
      "stats.subtitle": "A look back at two incredible semesters of achievements, growth, and community",
      "stats.overview.title": "At a Glance",
      "stats.overview.totalEvents": "Total Events",
      "stats.overview.attendanceEntries": "Attendance Entries",
      "stats.overview.totalMembers": "Total Members",
      "stats.types.title": "Events by Type",
      "stats.types.location": "By Location",
      "stats.types.action": "By Activity",
      "stats.semester.comparison": "Semester Comparison",
      "stats.semester.events": "Events",
      "stats.semester.attendance": "Attendance",
      "stats.semester.members": "Members",
      "stats.semester.avgPerEvent": "Avg / Event",
      "stats.semester.female": "Female",
      "stats.semester.male": "Male",
      "stats.department.title": "Department Performance",
      "stats.department.totalEvents": "Total Events",
      "stats.department.actualEvents": "Actual Events",
      "stats.department.attendance": "Attendance",
      "stats.highlights.title": "Highlights",
      "stats.highlights.topMembers": "Most Active Members",
      "stats.highlights.attendance": "attendance entries",
      "stats.all": "2025–2026",
      "stats.sem471": "Semester 471",
      "stats.sem472": "Semester 472",
      "stats.eventsByType": "Events by Activity Type",
      "stats.locationBreakdown": "Events by Location",
      "stats.attendanceLabel": "avg attendance",

      // Common UI Elements (ported from score-leaderboard-authentication)
      "common.loading": "Loading...",
      "common.close": "Close",
      "common.back": "Back",
      "common.continue": "Continue",
      "common.cancel": "Cancel",
      "common.confirm": "Confirm",

      // Sign Up Page
      "auth.signUp.badge": "New Account",
      "auth.signUp.title": "Create Account",
      "auth.signUp.description": "Create your GDG-Q account",
      "auth.signUp.universityId": "University ID",
      "auth.signUp.universityIdPlaceholder": "442106350",
      "auth.signUp.password": "Password",
      "auth.signUp.passwordPlaceholder": "Enter password",
      "auth.signUp.emailSuffix": "@qu.edu.sa",
      "auth.signUp.creating": "Creating Account...",
      "auth.signUp.submit": "Create Account",
      "auth.signUp.footer.text": "Already have an account?",
      "auth.signUp.footer.link": "Sign In",
      "auth.signUp.error.emailExists": "An account associated with this email already exists. Please sign in instead.",
      "auth.signUp.error.unexpected": "An unexpected error occurred. Please try again. or contact support.",
      "auth.signUp.orContinueWith": "Or continue with",
      "auth.signUp.continueWithGoogle": "Continue with Google",
      "auth.signUp.error.googleUnexpected": "Unable to continue with Google. Please try again.",

      // Sign In Page
      "auth.signIn.badge": "Sign In",
      "auth.signIn.title": "Welcome Back",
      "auth.signIn.description": "Sign in to your GDG-Q account",
      "auth.signIn.universityId": "University ID",
      "auth.signIn.universityIdPlaceholder": "442106350",
      "auth.signIn.password": "Password",
      "auth.signIn.passwordPlaceholder": "Enter password",
      "auth.signIn.emailSuffix": "@qu.edu.sa",
      "auth.signIn.signingIn": "Signing In...",
      "auth.signIn.submit": "Sign In",
      "auth.signIn.footer.text": "Don't have an account?",
      "auth.signIn.footer.link": "Sign Up",
      "auth.signIn.forgotPassword": "Forgot Password?",
      "auth.signIn.error.invalidCredentials": "Invalid email or password",
      "auth.signIn.error.sessionError": "Error creating session.",
      "auth.signIn.error.verificationCode": "Unable to send verification code.",
      "auth.signIn.error.signInFailed": "Unable to complete sign in.",
      "auth.signIn.error.unexpected": "An unexpected error occurred.",
      "auth.signIn.orContinueWith": "Or continue with",
      "auth.signIn.continueWithGoogle": "Continue with Google",
      "auth.signIn.error.googleUnexpected": "Unable to continue with Google. Please try again.",

      // Forgot Password Page
      "auth.forgotPassword.step1.title": "Forgot Password?",
      "auth.forgotPassword.step1.description": "Enter your University ID to receive a password reset code",
      "auth.forgotPassword.step1.universityId": "University ID",
      "auth.forgotPassword.step1.placeholder": "442106350",
      "auth.forgotPassword.step1.sending": "Sending Code...",
      "auth.forgotPassword.step1.submit": "Send Reset Code",
      "auth.forgotPassword.step1.footer.text": "Remember your password?",
      "auth.forgotPassword.step1.footer.link": "Sign In",
      "auth.forgotPassword.step1.error.noAccount": "No account found with this University ID",
      "auth.forgotPassword.step1.error.unexpected": "An unexpected error occurred.",

      "auth.forgotPassword.step2.title": "Enter Reset Code",
      "auth.forgotPassword.step2.description": "We sent a password reset code to {{email}}",
      "auth.forgotPassword.step2.verificationCode": "Verification Code",
      "auth.forgotPassword.step2.placeholder": "000000",
      "auth.forgotPassword.step2.continue": "Continue",
      "auth.forgotPassword.step2.back": "Back",
      "auth.forgotPassword.step2.resending": "Sending...",
      "auth.forgotPassword.step2.resend": "Resend code",
      "auth.forgotPassword.step2.resendCountdown": "Resend code in {{countdown}}s",
      "auth.forgotPassword.step2.goToOutlook": "Go to Outlook",
      "auth.forgotPassword.step2.error.resendFailed": "Failed to resend verification code",
      "auth.forgotPassword.step2.error.2faRequired": "Two-factor authentication is required. Please contact support.",
      "auth.forgotPassword.step2.error.unexpected": "An unexpected error occurred.",

      "auth.forgotPassword.step3.title": "Reset Password",
      "auth.forgotPassword.step3.description": "Enter your new password",
      "auth.forgotPassword.step3.newPassword": "New Password",
      "auth.forgotPassword.step3.placeholder": "Enter new password",
      "auth.forgotPassword.step3.resetting": "Resetting Password...",
      "auth.forgotPassword.step3.submit": "Reset Password",
      "auth.forgotPassword.step3.back": "Back",
      "auth.forgotPassword.step3.error.resetFailed": "Unable to reset password. Please try again.",
      "auth.forgotPassword.step3.error.unexpected": "An unexpected error occurred. Please try again or contact support.",

      // Verification Card
      "auth.verification.title.signUp": "Verify Your Email",
      "auth.verification.title.signIn": "Two-Factor Authentication",
      "auth.verification.description": "Enter the code sent to {{email}}",
      "auth.verification.verificationCode": "Verification Code",
      "auth.verification.placeholder": "000000",
      "auth.verification.verifying": "Verifying...",
      "auth.verification.submit": "Verify Code",
      "auth.verification.back.signUp": "Back to Sign Up",
      "auth.verification.back.signIn": "Back to Sign In",
      "auth.verification.resending": "Sending...",
      "auth.verification.resend": "Resend code",
      "auth.verification.resendCountdown": "Resend code in {{countdown}}s",
      "auth.verification.goToOutlook": "Go to Outlook",
      "auth.verification.error.noSignUpSession": "Sign up session not found",
      "auth.verification.error.noSignInSession": "Sign in session not found",
      "auth.verification.error.resendFailed": "Failed to resend verification code",
      "auth.verification.error.verificationFailed": "Unable to complete verification. Please try again.",
      "auth.verification.error.attemptFailed": "Verification failed. Please try again.",
      "auth.verification.error.unexpected": "An unexpected error occurred. Please try again. or contact support.",

      // User Profile Page (ported)
      "userProfile.title": "Welcome!",
      "userProfile.titleWithName": "Welcome, {{name}}",
      "userProfile.description": "You have successfully signed in. You can manage your account here",
      "userProfile.signOutTab": "sign-out",
      "userProfile.signOutTitle": "Sign Out",
      "userProfile.signOutDescription": "Are you sure you want to sign out",
      "userProfile.signOutButton": "Sign Out",

      // User Account Card
      "userCard.defaultName": "User",
      "userCard.clickToManage": "Click avatar to manage →",

      // Validation Messages
      "validation.universityId.mustBe9Digits": "University ID must be 9 digits",
      "validation.universityId.exactly9Digits": "University ID must be exactly 9 digits",
      "validation.password.required": "Password is required",
      "validation.password.minLength": "Password must be at least 8 characters",
      "validation.verificationCode.invalid": "Please enter a valid 6-digit verification code",

      // Onboarding Page
      "onboarding.title": "Complete Your Profile",
      "onboarding.description": "A few basics and you're registered with GDG Qassim.",
      "onboarding.section.personal": "Personal info",
      "onboarding.section.academic": "Academic status",
      "onboarding.section.contact": "Contact",
      "onboarding.uniId.label": "University ID",
      "onboarding.uniId.autoFilled": "Auto-filled",
      "onboarding.fullName.label": "Full Name",
      "onboarding.fullName.placeholder": "Example: Ibrahim Mohammed Bassam Al-Harbi",
      "onboarding.fullName.description": "This is the name that'll show on your membership card.",
      "onboarding.phone.label": "Phone Number",
      "onboarding.phone.placeholder": "05xxxxxxxx",
      "onboarding.phone.description": "A Saudi number, used for event updates and WhatsApp groups.",
      "onboarding.gender.label": "Gender",
      "onboarding.gender.male": "Male",
      "onboarding.gender.female": "Female",
      "onboarding.status.label": "Academic Status",
      "onboarding.status.quStudent": "Current QU student",
      "onboarding.status.graduate": "Graduate",
      "onboarding.status.highSchool": "High school student",
      "onboarding.status.description": "This decides what shows on your card and public profile.",
      "onboarding.level.label": "Academic Level",
      "onboarding.level.placeholder": "Choose level",
      "onboarding.level.graduated": "Graduated",
      "onboarding.college.label": "College",
      "onboarding.college.placeholder": "Choose college",
      "onboarding.college.other": "Other",
      "onboarding.collegeOther.label": "College Name",
      "onboarding.collegeOther.placeholder": "Example: College of Business Administration",
      "onboarding.highSchool.label": "School Name",
      "onboarding.highSchool.placeholder": "Example: Prince Sultan High School",
      "onboarding.email.label": "Personal Email",
      "onboarding.email.placeholder": "ahmad@gmail.com",
      "onboarding.email.description": "Your personal email, not your @qu.edu.sa one.",
      "onboarding.optional": "Optional",
      "onboarding.submit": "Complete Registration",
      "onboarding.submitting": "Completing Registration...",

      // Welcome Back Dialog (shown when an existing record is auto-linked by email)
      "welcomeBack.title": "Welcome back! 🎉",
      "welcomeBack.descriptionWithPoints": "We found activity already on file under this email and linked it to your account. You've already got {{points}} points on the board!",
      "welcomeBack.descriptionNoPoints": "We found activity already on file under this email and linked it to your account.",
      "welcomeBack.continue": "Let's go",

      // Onboarding Validation Messages
      "onboarding.validation.fullName.required": "Full name is required",
      "onboarding.validation.phone.length": "Phone number must be exactly 10 digits",
      "onboarding.validation.phone.format": "Phone number must start with 05 followed by 8 digits",
      "onboarding.validation.gender.required": "Please select a gender",
      "onboarding.validation.status.required": "Please select your academic status",
      "onboarding.validation.collegeOther.required": "Please enter your college name",
      "onboarding.validation.email.invalid": "Invalid email address",
      "onboarding.validation.email.notQuEmail": "Personal email cannot be a @qu.edu.sa address",

      // Onboarding Submission Errors
      "onboarding.submitError.emailInUse": "This email is already in use by another account. Please use a different email.",
      "onboarding.submitError.generic": "We couldn't finish setting up your account. Please try again.",

      // Common
      "version": "2.0"
    }
  },
  // always use the saudi arabian dialect for arabic words and phrases
  ar: {
    translation: {
      // Hero Section
      "hero.title.performance": "مجموعة قوقل للطلبة المطورين",
      "hero.subtitle": "سجل بفعاليتنا و تابع نقاطك وكن من المتصدرين 🏆",
      "hero.viewAllMembers": "شوف كل الأعضاء",
      "hero.viewDepartments": "شوف الأقسام",

      // Stats Cards  
      "stats.members.title": "الأعضاء",
      "stats.members.description": "أبطال التحدي",
      "stats.members.badge": "فعّال",
      "stats.departments.title": "الأقسام",
      "stats.departments.description": "فرقنا القوية",
      "stats.departments.badge": "فريق",
      "stats.points.title": "النقاط",
      "stats.points.description": "نقاطك المكتسبة",
      "stats.points.badge": "الإجمالي",
      "stats.events.title": "الفعاليات",
      "stats.events.description": "إجمالي الفعاليات",
      "stats.events.badge": "فعّال",

      // Leaderboard Section
      "leaderboard.title": "المتصدرين الآن",
      "leaderboard.subtitle": "شوف أفضل المنافسين بين الاعضاء والاقسام، التحديث لحظي",
      "leaderboard.topMembers": "أقوى الأعضاء",
      "leaderboard.topDepartments": "أقوى الأقسام",
      "leaderboard.viewAll": "الكل",
      "leaderboard.specializedDepts": "الأقسام التخصصية",
      "leaderboard.administrativeDepts": "الأقسام الإدارية",
      "leaderboard.points": "نقطة",
      "leaderboard.viewDetails": "التفاصيل",
      "leaderboard.details": "التفاصيل",
      "leaderboard.noMembersData": "ما فيه بيانات للأعضاء",
      "leaderboard.noSpecializedDeptsData": "ما فيه بيانات للأقسام التخصصية",
      "leaderboard.noAdministrativeDeptsData": "ما فيه بيانات للأقسام الإدارية",
      "leaderboard.showDetails": "إظهار التفاصيل",
      "leaderboard.hideDetails": "إخفاء التفاصيل",
      "leaderboard.show": "إظهار",
      "leaderboard.hide": "إخفاء",

      // Semester
      "semester.label": "الفصل الدراسي",
      "semester.all": "الكل",

      // Events Section
      "events.badge": "فعاليات مفتوحة",
      "events.title": "شارك في فعالياتنا",
      "events.subtitle": "عيش الحماس، شارك واكسب نقاط وتصدر القائمة!",
      "events.loading": "جاري تحميل الفعاليات...",
      "events.empty": "ما عندنا فعاليات متاحة حاليًا بس خلك قريب ترا فعالياتنا واجد 😉",
      "events.viewAll": "شف كل الفعاليات",

      // Navigation
      "nav.home": "الرئيسية",
      "nav.members": "الأعضاء",
      "nav.departments": "الأقسام",
      "nav.events": "الفعاليات",
      "nav.wallet": "المحفظة",
      "nav.stats": "إحصائيات",
      "nav.magazines": "المجلات",
      "nav.structure": "الهيكلة",
      "nav.howItWorks": "نظام النقاط",
      "nav.language": "اللغة",
      "nav.english": "الإنجليزية",
      "nav.arabic": "العربية",
      "nav.profile": "الملف الشخصي",
      "nav.admin": "لوحة الإدارة",

      // Profile Page
      "profile.pageTitle": "الملف الشخصي",
      "profile.gdgProfile": "بيانات GDG",
      "profile.accountManagement": "إدارة الحساب",
      "profile.eventsFeed": "سجل الفعاليات",
      "profile.eventsFeedPlaceholder": "سجل الفعاليات قريبًا...",
      "profile.attendedTitle": "فعاليات حضرتها",
      "profile.participatedTitle": "مسابقات وفعاليات شاركت فيها",
      "profile.noAttended": "ما حضرت أي فعالية للحين",
      "profile.noParticipated": "ما شاركت في أي فعالية للحين",
      "profile.downloadCertificate": "الشهادة",
      "profile.customize": "تخصيص",
      "profile.language": "اللغة",
      "profile.format": "الصيغة",
      "profile.download": "تحميل",
      "profile.langArabic": "العربية",
      "profile.langEnglish": "الإنجليزية",
      "profile.formatPdf": "PDF",
      "profile.formatPng": "PNG",
      "profile.downloading": "جاري التحميل...",
      "profile.downloadSuccess": "تم تحميل الشهادة بنجاح!",
      "profile.downloadError": "فشل تحميل الشهادة. جرب مرة ثانية.",
      "profile.certificateDisabledTooltip": "لازم تحضر كل الأيام عشان تقدر تحمل الشهادة",
      "profile.details": "التفاصيل",
      "profile.attendance": "الحضور",
      "profile.day": "يوم",
      "profile.attended": "حضرت",
      "profile.notAttended": "ما حضرت",

      // Footer CTA
      "footer.title": "جاهز تتصدر؟",
      "footer.subtitle": "شارك في الفعاليات، اجمع النقاط، وخلك دايم في القمة!",
      "footer.button": "شوف الفعاليات الجاية",

      // Members Page
      "members.backButton": "العودة للوحة التحكم",
      "members.heading": "قائمة الأعضاء",
      "members.subHeading": "عضو مرتبين حسب النقاط المكتسبة من خلال الأنشطة والإنجازات المختلفة",
      "members.findMembers": "ابحث عن الأعضاء",
      "members.searchPlaceholder": "ابحث عن الأعضاء بالاسم...",
      "members.rankings": "الترتيب",
      "members.performance": "ترتيب أداء الأعضاء الفردي",
      "members.of": "من",
      "members.found": "عضو",
      "members.noResults": "ما لقينا أعضاء يطابقون",
      "members.clearSearch": "امسح البحث",
      "members.noMembers": "ما فيه أعضاء متاحين",
      "members.viewYourRank": "شوف ترتيبك",

      // Events Page
      "events.heading": "الفعاليات",
      "events.subHeading": "اكتشف الفعاليات القادمة، سجل في الفعاليات المفتوحة، واستكشف الفعاليات السابقة",
      "events.openEvents": "الفعاليات المفتوحة",
      "events.pastEvents": "الفعاليات السابقة",
      "events.noOpenEvents": "ما فيه فعاليات متاحة للتسجيل حالياً. ارجع بعدين!",
      "events.noPastEvents": "الفعاليات السابقة راح تظهر هنا بعد ما تنتهي.",
      "events.SearchEvents": "ابحث عن فعالية...",
      "events.noResults": "ما لقينا الفعالية اللي تبحث عنها",

      // Departments Page
      "departments.backButton": "العودة للوحة التحكم",
      "departments.heading": "قائمة الأقسام",
      "departments.subHeading": "قسم مرتبين حسب النقاط المكتسبة من خلال التعاون والإنجازات الجماعية",
      "departments.administrative": "الأقسام الإدارية",
      "departments.administrativeDesc": "فرق الدعم والإدارة",
      "departments.specialized": "الأقسام التخصصية",
      "departments.specializedDesc": "الفرق العملية والتقنية",

      // Magazines Page
      "magazines.backButton": "العودة للوحة التحكم",
      "magazines.heading": "مجلات GDG",
      "magazines.noMagazines.title": "ما فيه مجلات متاحة",
      "magazines.noMagazines.desc": "ارجع بعدين لإصدارات المجلات الجديدة اللي تحتوي على آخر التقنيات والتطوير.",
      "magazines.pages": "صفحة",
      "magazines.pages_two": "صفحتين",
      "magazines.pages_few": "صفحات",
      "magazines.readPDF": "اقرأ PDF",
      "magazines.reader.backToMagazines": "العودة للمجلات",
      "magazines.reader.title": "قارئ المجلات",
      "magazines.reader.loading": "جاري تحميل القارئ...",
      "magazines.reader.notFound": "المجلة غير موجودة",

      // Magazines Section (Home)
      "magazines.section.title": "آخر المجلات",
      "magazines.section.subtitle": "اطلع على آخر إصداراتنا وخلك متابع مع المجتمع",
      "magazines.section.viewAll": "شوف كل المجلات",
      "magazines.section.empty": "ما فيه مجلات متاحة حالياً",

      // Club Structure Section
      "clubStructure.title": "هيكلة المجموعة",
      "clubStructure.subtitle": "تعرف على فريق القيادة واستكشف الأقسام اللي تقود مجتمعنا",
      "clubStructure.leadership": "القيادة",
      "clubStructure.presidents": "القادة",
      "clubStructure.vicePresident": "نائب الرئيس",
      "clubStructure.departmentsTitle": "الأقسام",
      "clubStructure.departments": "قسم",
      "clubStructure.specialized": "تخصصية",
      "clubStructure.administrative": "إدارية",
      "clubStructure.viewFullStructure": "شوف الهيكل الكامل",

      // Club Structure Page
      "clubStructurePage.title": "هيكلة المجموعة",
      "clubStructurePage.subtitle": "تعرف على الهيكل التنظيمي لنادي مطوري قوقل والأقسام المختلفة",
      "clubStructurePage.leadership": "القيادة",
      "clubStructurePage.specializedDepts": "الاقسام التخصصية",
      "clubStructurePage.specializedDesc": "أقسام متخصصة في مجالات التقنية والابتكار",
      "clubStructurePage.administrativeDepts": "الاقسام الإدارية",
      "clubStructurePage.administrativeDesc": "أقسام إدارية لتنظيم وتطوير أعمال النادي",
      "clubStructurePage.deptHead": "رئيس القسم:",
      "clubStructurePage.deptHeadMale": "رئيس القسم (طلاب)",
      "clubStructurePage.deptHeadFemale": "رئيس القسم (طالبات)",
      "clubStructurePage.deptMembers": "أعضاء القسم:",
      "clubStructurePage.presidents": "القادة",
      "clubStructurePage.vicePresident": "نائب الرئيس",

      // OLD specialized Departments
      "clubStructurePage.robotics": "الروبوتات",
      "clubStructurePage.ai": "الذكاء الاصطناعي وتحليل البيانات",
      "clubStructurePage.entrepreneurship": "ريادة الأعمال والتقنية",
      "clubStructurePage.esports": "الرياضات الإلكترونية",

      // NEW specialized Departments
      "clubStructurePage.blue": "الأزرق",
      "clubStructurePage.green": "الأخضر",
      "clubStructurePage.yellow": "الأصفر",
      
      // administrative Departments
      "clubStructurePage.development": "التطوير التقني",
      "clubStructurePage.programs": "إدارة البرامج واللوجستيات",
      "clubStructurePage.organization": "التنظيم والاجتماعات",
      "clubStructurePage.design": "التصميم",
      "clubStructurePage.media": "الإعلام والعلاقات العامة",
      "clubStructurePage.operations": "إدارة التشغيل",
      "clubStructurePage.activities": "الأنشطة العامة",

      "clubStructurePage.deptLeader": "الليدر:",
      "clubStructurePage.deptVice": "النائب:",
      "clubStructurePage.boardDirectors": "مجلس الإدارة",
      "clubStructurePage.innovation": "قسم الابتكار",
      "clubStructurePage.aiShort": "الذكاء الاصطناعي",
      "clubStructurePage.programsShort": "البرامج واللوجستيات",
      "clubStructurePage.loadError": "ما قدرنا نحمّل هيكلة النادي. حاول مرة ثانية.",
      "clubStructurePage.tryAgain": "حاول مرة ثانية",
      "clubStructurePage.noAssignments": "ما تم تعيين أحد إلى الآن",
      "clubStructurePage.noMembers": "ما تم إضافة أعضاء إلى الآن",
      "clubStructurePage.noDepartments": "ما فيه أقسام متاحة إلى الآن",

      // Auth
      "auth.login": "تسجيل الدخول",
      "auth.signup": "إنشاء حساب",
      "auth.loginAria": "سجل دخول لحسابك",
      "auth.signupAria": "أنشئ حساب جديد",
      "auth.signIn": "سجل دخول",
      "auth.joinUs": "انضم إلينا",
      "auth.cancel": "إلغاء",

      // Auth Required Dialog
      "authDialog.defaultTitle": "تسجيل الدخول مطلوب",
      "authDialog.defaultDescription": "لازم تسجل دخول أو تسوي حساب عشان تكمل.",

      // Event Signup
      "eventSignup.signUp": "سجل في الفعالية",
      "eventSignup.signedUp": "مسجل",
      "eventSignup.fillGoogleForm": "عبي نموذج جوجل",
      "eventSignup.somethingWentWrong": "⚠️ حدث خطأ ما",
      "eventSignup.signInRequired": "تسجيل الدخول مطلوب",
      "eventSignup.signInDescription": "لازم تسجل دخول أو تسوي حساب عشان تسجل في هذي الفعالية.",
      "eventSignup.confirmTitle": "تأكيد التسجيل",
      "eventSignup.confirmDescription": "متأكد إنك تبي تسجل في هذي الفعالية:",
      "eventSignup.cancel": "إلغاء",
      "eventSignup.signingUp": "جاري التسجيل...",
      "eventSignup.confirmAndFillForm": "أكد التسجيل وعبي النموذج",
      "eventSignup.confirm": "أكد التسجيل",
      "eventSignup.successToast": "تم تسجيلك بنجاح في",

      // Event Card
      "eventCard.viewEvent": "التفاصيل",
      "eventCard.viewDetails": " التفاصيل",

      // Attendance Page
      "attendance.processing": "جاري المعالجة...",
      "attendance.markingAttendance": "جاري تسجيل الحضور...",
      "attendance.confirmed": "تم تأكيد الحضور",
      "attendance.failed": "فشل تسجيل الحضور",
      "attendance.invalidLink": "رابط غير صالح",
      "attendance.signInRequired": "تسجيل الدخول مطلوب",
      "attendance.success": "تم تسجيل حضورك بنجاح!",
      "attendance.noToken": "ما فيه رمز حضور. استخدم رمز QR أو الرابط اللي أعطاك إياه منظم الفعالية.",
      "attendance.pleaseSignIn": "سجل دخول عشان تسجل حضورك.",
      "attendance.notEligible": "ما تقدر تسجل حضورك لهذي الفعالية.",
      "attendance.expired": "رابط الحضور منتهي أو غير صالح. اطلب رابط جديد من منظم الفعالية.",
      "attendance.serverError": "صار خطأ في السيرفر. جرب مرة ثانية بعدين.",
      "attendance.connectionError": "ما قدرنا نتصل بالسيرفر. تأكد من الإنترنت وجرب مرة ثانية.",
      "attendance.dialog.title": "تسجيل الدخول مطلوب",
      "attendance.dialog.description": "لازم تسجل دخول أو تسوي حساب عشان تسجل حضورك.",

      // Event Detail Page
      "eventDetail.status.open": "مفتوح للتسجيل",
      "eventDetail.status.active": "جاري",
      "eventDetail.status.closed": "منتهي",
      "eventDetail.status.openToJoin": "مفتوح للدخول",
      "eventDetail.locationType.online": "فعالية أونلاين",
      "eventDetail.locationType.onsite": "فعالية حضورية",
      "eventDetail.locationType.none": "بدون موقع",
      "eventDetail.daily": "يومياً",
      "eventDetail.days": "أيام",
      "eventDetail.description": "الوصف",
      "eventDetail.noDescription": "ما فيه وصف لهذي الفعالية.",
      "eventDetail.noImage": "ما فيه صورة للفعالية",
      "eventDetail.joinMeeting": "ادخل الاجتماع",
      "eventDetail.meetingLink": "رابط الاجتماع",
      "eventDetail.addToCalendar": "أضفها للتقويم",

      // Member Detail Page
      "memberDetail.backToMembers": "العودة للأعضاء",
      "memberDetail.memberProfile": "ملف العضو",
      "memberDetail.member": "عضو",
      "memberDetail.totalPoints": "إجمالي النقاط",
      "memberDetail.pointsHistory": "سجل النقاط",
      "memberDetail.detailedLog": "سجل تفصيلي لجميع النقاط المكتسبة",
      "memberDetail.points": "نقطة",
      "memberDetail.noHistory": "ما فيه سجل نقاط متاح لهذا العضو.",

      // Department Detail Page
      "departmentDetail.backToDepartments": "العودة للأقسام",
      "departmentDetail.departmentProfile": "ملف القسم",
      "departmentDetail.department": "قسم",
      "departmentDetail.totalPoints": "إجمالي النقاط",
      "departmentDetail.pointsHistory": "سجل النقاط",
      "departmentDetail.detailedLog": "سجل تفصيلي لجميع النقاط المكتسبة للقسم",
      "departmentDetail.points": "نقطة",
      "departmentDetail.noHistory": "ما فيه سجل نقاط متاح لهذا القسم.",

      // Footer
      "footer.about.title": "عن GDG القصيم",
      "footer.about.description": "مجموعة قوقل للطلبة المطورين مجتمع طلابي يسعى لبناء مجتمع تقني مبتكر من خلال ورش العمل والفعاليات والمشاريع الجماعية.",
      "footer.quickLinks.title": "روابط سريعة",
      "footer.connect.title": "تواصل معنا",
      "footer.developers.title": "المطورين",
      "footer.developers.builtWith": "تم بناء الموقع بـ ❤️ من",
      "footer.social.joinUs": "انضم لمجتمعنا",
      "footer.social.telegram": "مجموعة التلقرام",
      "footer.social.discord": "سيرفر الديسكورد",
      "footer.social.twitter": "تطبيق X",
      "footer.social.hashtag": "هاشتاقنا على تويتر",
      "footer.social.tiktok": "تيك توك",
      "footer.social.email": "تواصل معنا",
      "footer.social.gdgSite": "موقع GDG الرسمي",
      "footer.copyright": "© GDG-Q 2026",
      "footer.privacy": "سياسة الخصوصية",
      "footer.terms": "شروط الخدمة",

      // Feedback
      "feedback.card.title": "رأيك يهمنا",

      // Not Found Pages
      "notFound.pageTitle": "الصفحة غير موجودة",
      "notFound.pageDescription": "الصفحة اللي تدور عليها مو موجودة أو تم نقلها",
      "notFound.backHome": "ارجع للرئيسية",
      
      "notFound.eventTitle": "الفعالية غير موجودة",
      "notFound.eventDescription": "الفعالية اللي تدور عليها مو موجودة أو تم حذفها",
      "notFound.backToEvents": "ارجع للفعاليات",

      // How / Points System Page
      "how.title": "نظام التقييم والمتابعة للنقاط",
      "how.subtitle": "لمجموعة قوقل للطلبة المطورين",
      "how.generalIdea": "الفكرة العامة",
      "how.generalIdea.desc": "نقاط قوقل تعكس مساهمات الأعضاء والأقسام، حيث يتم تقدير كل جهد وإنجاز بشكل واضح وملموس. مشاركتك معنا تصنع الفرق، وكل عضو له دور فريد وبصمة خاصة.",
      "how.monthly": "شهرياً",
      "how.monthly.topMembers": "ثلاث أعضاء الشهر المميزين",
      "how.monthly.topDept": "قسم الشهر المبدع",
      "how.endOfTerm": "نهاية الفصل",
      "how.endOfTerm.top5": "تكريم أفضل 5 أعضاء (قابلة للزيادة إلى 10)",
      "how.endOfTerm.specialized": "تكريم القسم التخصصي المتميز",
      "how.endOfTerm.administrative": "تكريم القسم الإداري المتميز",
      "how.note.multipliers": "كل نشاط له نقاط أساسية + مضاعفات (حسب الحضور والجودة والتزام الوقت… إلخ).",
      "how.note.bothTypes": "فيه نقاط للأقسام (نقاط قسم) وفيه نقاط للأعضاء (نقاط عضو).",
      "how.memberPoints": "نقاط الأعضاء",
      "how.memberPoints.subtitle": "حضور / مشاركة / تنظيم",
      "how.tableHeader.action": "الفعل",
      "how.tableHeader.points": "النقاط",
      "how.deptPoints": "نقاط الأقسام",
      "how.deptPoints.subtitle": "أنواع الأنشطة والنقاط الأساسية",
      "how.tableHeader.type": "النوع",
      "how.tableHeader.definition": "التعريف",
      "how.memberAction.0": "حضور دورة حضورية",
      "how.memberAction.1": "حضور دورة أونلاين",
      "how.memberAction.2": "حضور معسكر (لكل يوم)",
      "how.memberAction.3": "حضور معسكر أونلاين (لكل يوم)",
      "how.memberAction.4": "حضور حدث ضخم (300+ حضور)",
      "how.memberAction.5": "حضور لقاء تقني/جلسة",
      "how.memberAction.6": "المشاركة في مسابقة خارجية",
      "how.memberAction.7": "التأهل/المراكز المتقدمة في مسابقة",
      "how.memberAction.8": "الفوز/المركز الأول خارجياً",
      "how.memberAction.9": "تقديم جلسة قصيرة (≤ 20 دقيقة)",
      "how.memberAction.10": "تقديم دورة كاملة",
      "how.memberAction.11": "تنظيم/تطوع في حدث (شِفت)",
      "how.memberAction.12": "المساهمة في تصميم معتمد",
      "how.memberAction.13": "كتابة مقال/فيديو تعليمي معتمد",
      "how.memberAction.14": "المشاركة في مشروع عملي مُعلن",
      "how.deptType.0": "دورة حضورية",
      "how.deptType.1": "دورة أونلاين",
      "how.deptType.2": "معسكر تدريبي",
      "how.deptType.3": "معسكر أونلاين",
      "how.deptType.4": "حدث ضخم (300+ حضور)",
      "how.deptType.5": "مساحة تويتر",
      "how.deptType.6": "لقاء تقني/جلسة شهرية",
      "how.deptType.7": "مسابقة داخلية (تنظيم)",
      "how.deptType.8": "مشاركة بمسابقة خارجية",
      "how.deptType.9": "مشروع عملي/منتج",
      "how.deptType.10": "فعالية تعريفية/جناح",
      "how.deptType.11": "شراكة/رعاية",
      "how.deptType.12": "محتوى تعليمي منشور",
      "how.deptDesc.0": "تنظيم دورة حضورية",
      "how.deptDesc.1": "تنظيم دورة عن بُعد",
      "how.deptDesc.2": "معسكر حضوري (لكل يوم)",
      "how.deptDesc.3": "معسكر تدريبي عن بُعد (لكل يوم)",
      "how.deptDesc.4": "فعالية كبيرة بحضور 300 شخص أو أكثر",
      "how.deptDesc.5": "استضافة مساحة صوتية على تويتر",
      "how.deptDesc.6": "جلسة تبادل معرفة/نقاش",
      "how.deptDesc.7": "مسابقة ينظمها القسم",
      "how.deptDesc.8": "يُحتسب للقسم مرة واحدة لكل فريق ممثل",
      "how.deptDesc.9": "تسليم مشروع مع مستودع عام + عرض",
      "how.deptDesc.10": "ركن/تعريف بالنادي داخل الجامعة",
      "how.deptDesc.11": "اتفاق يدعم نشاط/جوائز/مكان",
      "how.deptDesc.12": "مقال/فيديو تحت هوية النادي",

      // Profile Form
      "profileForm.uniId": "الرقم الجامعي",
      "profileForm.name": "الاسم",
      "profileForm.name.placeholder": "مثال: ابراهيم محمد بسام الحربي",
      "profileForm.phone": "رقم الجوال",
      "profileForm.gender": "الجنس",
      "profileForm.gender.male": "ذكر",
      "profileForm.gender.female": "أنثى",
      "profileForm.level": "المستوى الدراسي",
      "profileForm.level.placeholder": "اختر المستوى",
      "profileForm.level.graduated": "خريج",
      "profileForm.college": "الكلية",
      "profileForm.college.placeholder": "اختر الكلية",
      "profileForm.collegeOther": "اسم الكلية",
      "profileForm.collegeOther.placeholder": "أدخل اسم كليتك",
      "profileForm.email": "البريد الشخصي",
      "profileForm.email.hint": "البريد الشخصي وليس المنتهي بـ qu.edu.sa@",
      "profileForm.email.lockedHint": "هذا بريد حساب Google اللي دخلت فيه، ما تقدر تغيره من هنا.",
      "profileForm.uniId.notLinked": "غير مرتبط (تم الدخول عبر Google)",
      "profileForm.locked": "مقفل",
      "profileForm.saving": "جاري الحفظ...",
      "profileForm.save": "حفظ التغييرات",
      "walletTheme.label": "لون البطاقة",
      "walletTheme.success": "تم تحديث لون بطاقتك",
      "walletTheme.error": "ما قدرنا نحدث لون البطاقة، حاول مرة ثانية",
      "profileForm.errors.nameRequired": "الاسم مطلوب",
      "profileForm.errors.phoneRequired": "رقم الجوال مطلوب",
      "profileForm.errors.phoneInvalid": "رقم الجوال لازم يبدأ بـ 05 ويتكون من 10 أرقام",
      "profileForm.errors.collegeOtherRequired": "أدخل اسم كليتك",
      "profileForm.errors.emailRequired": "البريد الإلكتروني مطلوب",
      "profileForm.errors.emailInvalid": "بريد إلكتروني غير صالح",
      "profileForm.errors.emailUniversity": "البريد الشخصي وليس الجامعي اللي ينتهي بـ @qu.edu.sa",
      "profileForm.toast.clerkUpdatedBackendFailed": "تم تحديث البيانات بس صارت مشكلة في المزامنة",
      "profileForm.toast.emailConflict": "هذا البريد مستخدم فعلاً من حساب ثاني. جرب بريد مختلف.",
      "profileForm.toast.success": "تم تحديث الملف الشخصي بنجاح!",
      "profileForm.toast.error": "صار خطأ غير متوقع. جرب مرة ثانية.",

      // Error Display
      "error.connection": "خطأ في الاتصال",
      "error.notFound": "غير موجود",
      "error.unauthorized": "غير مصرح",
      "error.error": "خطأ",
      "error.somethingWentWrong": "صار شيء غلط",
      "error.connectionMessage": "تأكد من الإنترنت وجرب مرة ثانية.",
      "error.notFoundMessage": "المحتوى اللي تدور عليه مو موجود.",
      "error.unauthorizedMessage": "لازم تسجل دخول عشان تشوف هذي الصفحة.",
      "error.unexpectedMessage": "صار خطأ غير متوقع.",
      "error.tryAgain": "جرب مرة ثانية",

      // Google Form
      "googleForm.noFormUrl": "ما فيه رابط نموذج",
      "googleForm.loading": "جاري التحميل…",

      // Stats Page
      "stats.title": "ملخص الموسم",
      "stats.subtitle": "نظرة على فصلين دراسيين مليان إنجازات ونمو ومجتمع",
      "stats.overview.title": "نظرة عامة",
      "stats.overview.totalEvents": "إجمالي الفعاليات",
      "stats.overview.attendanceEntries": "سجلات الحضور",
      "stats.overview.totalMembers": "إجمالي الأعضاء",
      "stats.types.title": "الفعاليات حسب النوع",
      "stats.types.location": "حسب الموقع",
      "stats.types.action": "حسب النشاط",
      "stats.semester.comparison": "مقارنة الفصول",
      "stats.semester.events": "الفعاليات",
      "stats.semester.attendance": "الحضور",
      "stats.semester.members": "الأعضاء",
      "stats.semester.avgPerEvent": "متوسط/فعالية",
      "stats.semester.female": "إناث",
      "stats.semester.male": "ذكور",
      "stats.department.title": "أداء الأقسام",
      "stats.department.totalEvents": "إجمالي الفعاليات",
      "stats.department.actualEvents": "فعاليات فعلية",
      "stats.department.attendance": "الحضور",
      "stats.highlights.title": "أبرز المحطات",
      "stats.highlights.topMembers": "الأعضاء الأكثر نشاطاً",
      "stats.highlights.attendance": "سجلات حضور",
      "stats.all": "٢٠٢٥–٢٠٢٦",
      "stats.sem471": "الفصل ٤٧١",
      "stats.sem472": "الفصل ٤٧٢",
      "stats.eventsByType": "الفعاليات حسب نوع النشاط",
      "stats.locationBreakdown": "الفعاليات حسب الموقع",
      "stats.attendanceLabel": "متوسط الحضور",

      // Common UI Elements (ported from score-leaderboard-authentication)
      "common.loading": "جاري التحميل...",
      "common.close": "إغلاق",
      "common.back": "رجوع",
      "common.continue": "متابعة",
      "common.cancel": "إلغاء",
      "common.confirm": "تأكيد",

      // Sign Up Page
      "auth.signUp.badge": "حساب جديد",
      "auth.signUp.title": "إنشاء حساب",
      "auth.signUp.description": "أنشئ حسابك في GDG-Q",
      "auth.signUp.universityId": "الرقم الجامعي",
      "auth.signUp.universityIdPlaceholder": "442106350",
      "auth.signUp.password": "كلمة المرور",
      "auth.signUp.passwordPlaceholder": "أدخل كلمة المرور",
      "auth.signUp.emailSuffix": "@qu.edu.sa",
      "auth.signUp.creating": "جاري إنشاء الحساب...",
      "auth.signUp.submit": "إنشاء حساب",
      "auth.signUp.footer.text": "عندك حساب من قبل؟",
      "auth.signUp.footer.link": "تسجيل الدخول",
      "auth.signUp.error.emailExists": "البريد الإلكتروني موجود من قبل. سجّل دخول بدلاً من ذلك.",
      "auth.signUp.error.unexpected": "حدث خطأ غير متوقع. حاول مرة ثانية أو تواصل مع الدعم الفني.",
      "auth.signUp.orContinueWith": "أو تابع باستخدام",
      "auth.signUp.continueWithGoogle": "المتابعة باستخدام Google",
      "auth.signUp.error.googleUnexpected": "تعذّرت المتابعة باستخدام Google. حاول مرة ثانية.",

      // Sign In Page
      "auth.signIn.badge": "تسجيل الدخول",
      "auth.signIn.title": "مرحباً بك من جديد",
      "auth.signIn.description": "سجّل دخولك لحسابك في GDG-Q",
      "auth.signIn.universityId": "الرقم الجامعي",
      "auth.signIn.universityIdPlaceholder": "442106350",
      "auth.signIn.password": "كلمة المرور",
      "auth.signIn.passwordPlaceholder": "أدخل كلمة المرور",
      "auth.signIn.emailSuffix": "@qu.edu.sa",
      "auth.signIn.signingIn": "جاري تسجيل الدخول...",
      "auth.signIn.submit": "تسجيل الدخول",
      "auth.signIn.footer.text": "ما عندك حساب؟",
      "auth.signIn.footer.link": "إنشاء حساب",
      "auth.signIn.forgotPassword": "نسيت كلمة المرور؟",
      "auth.signIn.error.invalidCredentials": "البريد الإلكتروني أو كلمة المرور غير صحيحة",
      "auth.signIn.error.sessionError": "خطأ في إنشاء الجلسة.",
      "auth.signIn.error.verificationCode": "تعذر إرسال رمز التحقق.",
      "auth.signIn.error.signInFailed": "تعذر إكمال تسجيل الدخول.",
      "auth.signIn.error.unexpected": "حدث خطأ غير متوقع.",
      "auth.signIn.orContinueWith": "أو تابع باستخدام",
      "auth.signIn.continueWithGoogle": "المتابعة باستخدام Google",
      "auth.signIn.error.googleUnexpected": "تعذّرت المتابعة باستخدام Google. حاول مرة ثانية.",

      // Forgot Password Page
      "auth.forgotPassword.step1.title": "نسيت كلمة المرور؟",
      "auth.forgotPassword.step1.description": "أدخل رقمك الجامعي عشان نرسل لك رمز إعادة تعيين كلمة المرور",
      "auth.forgotPassword.step1.universityId": "الرقم الجامعي",
      "auth.forgotPassword.step1.placeholder": "442106350",
      "auth.forgotPassword.step1.sending": "جاري الإرسال...",
      "auth.forgotPassword.step1.submit": "إرسال رمز إعادة التعيين",
      "auth.forgotPassword.step1.footer.text": "تذكرت كلمة المرور؟",
      "auth.forgotPassword.step1.footer.link": "تسجيل الدخول",
      "auth.forgotPassword.step1.error.noAccount": "ما حصلنا حساب بهذا الرقم الجامعي",
      "auth.forgotPassword.step1.error.unexpected": "حدث خطأ غير متوقع.",

      "auth.forgotPassword.step2.title": "أدخل رمز إعادة التعيين",
      "auth.forgotPassword.step2.description": "أرسلنا رمز إعادة تعيين كلمة المرور إلى {{email}}",
      "auth.forgotPassword.step2.verificationCode": "رمز التحقق",
      "auth.forgotPassword.step2.placeholder": "000000",
      "auth.forgotPassword.step2.continue": "متابعة",
      "auth.forgotPassword.step2.back": "رجوع",
      "auth.forgotPassword.step2.resending": "جاري الإرسال...",
      "auth.forgotPassword.step2.resend": "إعادة إرسال الرمز",
      "auth.forgotPassword.step2.resendCountdown": "إعادة إرسال الرمز بعد {{countdown}} ثانية",
      "auth.forgotPassword.step2.goToOutlook": "فتح Outlook",
      "auth.forgotPassword.step2.error.resendFailed": "فشل في إعادة إرسال رمز التحقق",
      "auth.forgotPassword.step2.error.2faRequired": "المصادقة الثنائية مطلوبة. تواصل مع الدعم الفني.",
      "auth.forgotPassword.step2.error.unexpected": "حدث خطأ غير متوقع.",

      "auth.forgotPassword.step3.title": "إعادة تعيين كلمة المرور",
      "auth.forgotPassword.step3.description": "أدخل كلمة المرور الجديدة",
      "auth.forgotPassword.step3.newPassword": "كلمة المرور الجديدة",
      "auth.forgotPassword.step3.placeholder": "أدخل كلمة المرور الجديدة",
      "auth.forgotPassword.step3.resetting": "جاري إعادة التعيين...",
      "auth.forgotPassword.step3.submit": "إعادة تعيين كلمة المرور",
      "auth.forgotPassword.step3.back": "رجوع",
      "auth.forgotPassword.step3.error.resetFailed": "تعذر إعادة تعيين كلمة المرور. حاول مرة ثانية.",
      "auth.forgotPassword.step3.error.unexpected": "حدث خطأ غير متوقع. حاول مرة ثانية أو تواصل مع الدعم الفني.",

      // Verification Card
      "auth.verification.title.signUp": "تحقق من بريدك الإلكتروني",
      "auth.verification.title.signIn": "المصادقة الثنائية",
      "auth.verification.description": "أدخل الرمز المُرسل إلى {{email}}",
      "auth.verification.verificationCode": "رمز التحقق",
      "auth.verification.placeholder": "000000",
      "auth.verification.verifying": "جاري التحقق...",
      "auth.verification.submit": "تحقق من الرمز",
      "auth.verification.back.signUp": "رجوع لإنشاء الحساب",
      "auth.verification.back.signIn": "رجوع لتسجيل الدخول",
      "auth.verification.resending": "جاري الإرسال...",
      "auth.verification.resend": "إعادة إرسال الرمز",
      "auth.verification.resendCountdown": "إعادة إرسال الرمز بعد {{countdown}} ثانية",
      "auth.verification.goToOutlook": "فتح Outlook",
      "auth.verification.error.noSignUpSession": "ما حصلنا جلسة إنشاء الحساب",
      "auth.verification.error.noSignInSession": "ما حصلنا جلسة تسجيل الدخول",
      "auth.verification.error.resendFailed": "فشل في إعادة إرسال رمز التحقق",
      "auth.verification.error.verificationFailed": "تعذر إكمال التحقق. حاول مرة ثانية.",
      "auth.verification.error.attemptFailed": "فشل التحقق. حاول مرة ثانية.",
      "auth.verification.error.unexpected": "حدث خطأ غير متوقع. حاول مرة ثانية أو تواصل مع الدعم الفني.",

      // User Profile Page (ported)
      "userProfile.title": "حياك الله!",
      "userProfile.titleWithName": "حياك، {{name}}",
      "userProfile.description": "تم تسجيل دخولك بنجاح. تقدر تتحكم بحسابك من هنا",
      "userProfile.signOutTab": "sign-out",
      "userProfile.signOutTitle": "تسجيل الخروج",
      "userProfile.signOutDescription": "متأكد إنك تبي تسجّل خروج؟",
      "userProfile.signOutButton": "تسجيل الخروج",

      // User Account Card
      "userCard.defaultName": "المستخدم",
      "userCard.clickToManage": "اضغط على الصورة للإدارة ←",

      // Validation Messages
      "validation.universityId.mustBe9Digits": "الرقم الجامعي لازم يكون 9 أرقام",
      "validation.universityId.exactly9Digits": "الرقم الجامعي لازم يكون 9 أرقام بالضبط",
      "validation.password.required": "كلمة المرور مطلوبة",
      "validation.password.minLength": "كلمة المرور لازم تكون 8 أحرف على الأقل",
      "validation.verificationCode.invalid": "أدخل رمز تحقق صحيح مكون من 6 أرقام",

      // Onboarding Page
      "onboarding.title": "أكمل بياناتك",
      "onboarding.description": "بيانات بسيطة وتصير عضو مسجل في نادي قوقل للطلبة المطورين بجامعة القصيم.",
      "onboarding.section.personal": "المعلومات الشخصية",
      "onboarding.section.academic": "الحالة الدراسية",
      "onboarding.section.contact": "التواصل",
      "onboarding.uniId.label": "الرقم الجامعي",
      "onboarding.uniId.autoFilled": "معبّى تلقائياً",
      "onboarding.fullName.label": "الاسم الرباعي",
      "onboarding.fullName.placeholder": "مثال: ابراهيم محمد بسام الحربي",
      "onboarding.fullName.description": "هذا الاسم اللي بيظهر على بطاقة عضويتك.",
      "onboarding.phone.label": "رقم الجوال",
      "onboarding.phone.placeholder": "05xxxxxxxx",
      "onboarding.phone.description": "نستخدمه لتنبيهك بالفعاليات ومجموعات الواتساب.",
      "onboarding.gender.label": "الجنس",
      "onboarding.gender.male": "ذكر",
      "onboarding.gender.female": "أنثى",
      "onboarding.status.label": "حالتك الدراسية",
      "onboarding.status.quStudent": "طالب حالي بجامعة القصيم",
      "onboarding.status.graduate": "خريج",
      "onboarding.status.highSchool": "طالب ثانوي",
      "onboarding.status.description": "تحدد لنا وش نعرضه في بطاقتك وملفك الشخصي.",
      "onboarding.level.label": "المستوى الدراسي",
      "onboarding.level.placeholder": "اختر المستوى",
      "onboarding.level.graduated": "خريج",
      "onboarding.college.label": "الكلية",
      "onboarding.college.placeholder": "اختر الكلية",
      "onboarding.college.other": "أخرى",
      "onboarding.collegeOther.label": "اسم الكلية",
      "onboarding.collegeOther.placeholder": "مثال: كلية إدارة الأعمال",
      "onboarding.highSchool.label": "اسم المدرسة",
      "onboarding.highSchool.placeholder": "مثال: ثانوية الأمير سلطان",
      "onboarding.email.label": "البريد الشخصي",
      "onboarding.email.placeholder": "ahmad@gmail.com",
      "onboarding.email.description": "لازم يكون بريدك الشخصي، مو بريدك الجامعي (qu.edu.sa@).",
      "onboarding.optional": "اختياري",
      "onboarding.submit": "كمّل التسجيل",
      "onboarding.submitting": "جاري إكمال تسجيلك...",

      // Welcome Back Dialog
      "welcomeBack.title": "هلا فيك! 🎉",
      "welcomeBack.descriptionWithPoints": "لقينا نشاط سابق مسجل على نفس الإيميل وربطناه بحسابك. عندك الحين {{points}} نقطة في رصيدك!",
      "welcomeBack.descriptionNoPoints": "لقينا نشاط سابق مسجل على نفس الإيميل وربطناه بحسابك.",
      "welcomeBack.continue": "يلا نبدأ",

      // Onboarding Validation Messages
      "onboarding.validation.fullName.required": "الاسم الكامل مطلوب",
      "onboarding.validation.phone.length": "رقم الجوال لازم يكون 10 أرقام بالضبط",
      "onboarding.validation.phone.format": "رقم الجوال لازم يبدأ بـ 05 متبوعاً بـ 8 أرقام",
      "onboarding.validation.gender.required": "الجنس مطلوب",
      "onboarding.validation.status.required": "الحالة الدراسية مطلوبة",
      "onboarding.validation.collegeOther.required": "اسم الكلية مطلوب",
      "onboarding.validation.email.invalid": "البريد الإلكتروني غير صحيح",
      "onboarding.validation.email.notQuEmail": "البريد الشخصي ما يصير من نطاق qu.edu.sa@",

      // Onboarding Submission Errors
      "onboarding.submitError.emailInUse": "هذا البريد الإلكتروني مستخدم في حساب ثاني. جرّب بريد إلكتروني مختلف.",
      "onboarding.submitError.generic": "ما قدرنا نكمّل إعداد حسابك. حاول مرة ثانية.",

      // Common
      "version": "2.0"
    }
  }
};
